...
Mongo.Collection.prototype.logDriver = function(collection, docIdentifier) {
  ...
  this.after.update(function(userId, doc, fieldNames, modifier) {
    ...
    if (_.intersection(modifiedKeys, self.log.keys).length) {
      logOperation(OPERATION_UPDATE, description, userId, doc, this, modifier);
    }
  });
...
Meteor.Collection.prototype.logs = function(logs){
  debugger;
  if (logs.update){
    for (let key in logs.update){
      
      let [field, operator] = key.split(' ');
      console.log('field', field)      
      console.log('operator', operator);      
      console.log(logs.update[key]());
    }
    ...
    // let description = DEFAULT_UPDATE_DESCRIPTION;
    // modifiedKeys.forEach(function(key) {
    //   if (key === 'projectsId' && !!modifier['$pull']) {
    //     description = 'unliked a project';
    //   }
    // })
