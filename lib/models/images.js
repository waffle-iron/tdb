...
function createAboutBox(fileObj, readStream, writeStream) {
  gm(readStream, fileObj.name()).resize(ABOUT_BOX_WIDTH, ABOUT_BOX_HEIGHT).stream().pipe(writeStream);
}
if (Meteor.isServer) {
  ...
  let aboutBoxStore = new FS.Store.S3('aboutBoxes', {
    region: 'sa-east-1',
    transformWrite: createAboutBox,
    accessKeyId: Meteor.settings.AWSAccessKeyId,
    secretAccessKey: Meteor.settings.AWSSecretAccessKey,
    bucket: Meteor.settings.AWSBucket,
    folder: 'about-boxes'
  ...
  Images = new FS.Collection('Images', {
    stores: [imageStore, thumbStore, cardStore, aboutBoxStore],
    filter: {
      allow: {
        contentTypes: ['image/*']
      }
