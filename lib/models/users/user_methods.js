...
// ********************************
Meteor.methods({
  'users/addProject': function(targetUserId, projectId) {
    check(targetUserId, String);
    check(projectId, String);

    if (isAdmin()) {
      if (Roles.userIsInRole(targetUserId, ['editor'])) {
        ...
        throw new Meteor.Error(403, 'Usuário não possui permissão para editar projetos.');
      }
    } else {
      throw new Meteor.Error(403, 'Não tem permissão');
    }
  },
  'users/removeProject': function(targetUserId, projectId) {
    check(targetUserId, String);
    check(projectId, String);
