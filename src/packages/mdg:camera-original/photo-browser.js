...
Template.viewfinder.rendered = function() {
  ...
  var viewfinderWidth = 640;
  var viewfinderHeight = 360;
  var resized = false;
...
MeteorCamera.getPicture = function (options, callback) {
  ...
  desiredWidth = options.width || 360;

  // Canvas#toDataURL takes the quality as a 0-1 value, not a percentage
  quality = (options.quality || 49) / 100;

  if (desiredHeight * 16 / 9 > desiredWidth) {
    canvasWidth = desiredHeight * 16 / 9;
    canvasHeight = desiredHeight;
  } else {
    canvasHeight = desiredWidth * 9 / 16;
    canvasWidth = desiredWidth;
  }

  canvasWidth = Math.round(desiredWidth);
  canvasHeight = Math.round(desiredHeight);
